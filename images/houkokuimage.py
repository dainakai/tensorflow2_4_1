...
paths = glob.glob("./currentcnn/*")
# imageNames = paths.replace("./currentcnn/",'')
imageNames = []
for ii in range(len(paths)):
    tmp = paths[ii].replace("./currentcnn/","")
...
for ii in range(len(paths)):
    ...
    set yrange[0:{size}];
    set tics out;
    set output "currentcnn_houkoku/{imageNames[ii]}";
    plot "currentcnn/{imageNames[ii]}" binary filetype=png origin=(0.5,0.5) with rgbimage;
    '''
